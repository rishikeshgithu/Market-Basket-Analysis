...
# Display frequency and support of sku_names
st.subheader("Frequency and Support of sku_names:")
for sku_name, count in sku_name_counts.items():
    sku_name_support_value = sku_name_support.get(sku_name, 0)
    st.write(f"{sku_name}: Frequency = {count}, Support = {sku_name_support_value:.20f}")

# Display frequency and support of brand_names
st.subheader("Frequency and Support of brand_names:")
for brand_name, count in brand_name_counts.items():
    brand_name_support_value = brand_name_support.get(brand_name, 0)
    st.write(f"{brand_name}: Frequency = {count}, Support = {brand_name_support_value:.20f}")

# Display frequency and support of categories
st.subheader("Frequency and Support of categories:")
for category, count in category_counts.items():
    category_support_value = category_support.get(category, 0)
    st.write(f"{category}: Frequency = {count}, Support = {category_support_value:.20f}")

# Display support of sku_name bought with selected_antecedent
st.subheader(f"Support of {antecedent_type} bought with '{selected_antecedent}':")
for sku_name, support in sku_name_bought_with_support.items():
    st.write(f"{sku_name}: {support:.20f}")

# Display support of brand_name bought with selected_antecedent
st.subheader(f"Support of brand_name bought with '{selected_antecedent}':")
for brand_name, support in brand_name_bought_with_support.items():
    st.write(f"{brand_name}: {support:.20f}")

# Display support of Category bought with selected_antecedent
st.subheader(f"Support of Category bought with '{selected_antecedent}':")
for category, support in category_bought_with_support.items():
    st.write(f"{category}: {support:.20f}")

# Display confidence, lift, and conviction of Prod_1 -> entity bought together
for entity in ['sku_name', 'brand_name', 'Category']:
    st.subheader(f"Confidence, Lift, and Conviction of Prod_1 -> {entity} bought together:")
    for item, metrics in confidence_lift_conviction[entity].items():
        st.write(f"{item}:")
        st.write(f"  Confidence: {metrics['Confidence']:.20f}")
        st.write(f"  Lift: {metrics['Lift']:.20f}")
        st.write(f"  Conviction: {metrics['Conviction']:.20f}")
