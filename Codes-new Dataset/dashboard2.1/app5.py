...
# Function to calculate support for each item
def calculate_support(data, item_type):
    total_transactions = data['receipt_id'].nunique()
    support_dict = {}
    for item in data[item_type].unique():
        transactions_with_item = data[data[item_type] == item]['receipt_id'].nunique()
        support = transactions_with_item / total_transactions
...
# Create a button to navigate to the support for all items page
if st.button('View Support for All Items'):
    st.session_state.show_support_page = True
else:
    st.session_state.show_support_page = False

if st.session_state.show_support_page:
    st.header("Support for All Items")
    support_type = st.selectbox("Select type for support calculation:", ['sku_name', 'brand_name', 'Category', 'Group'])
    support_dict = calculate_support(data, support_type)
    support_df = pd.DataFrame.from_dict(support_dict, orient='index', columns=['Support'])
    support_df = support_df.sort_values(by='Support', ascending=False)
    st.write(support_df)
    
    # Add a back button to return to the main analysis page
    ...
    if filter_a and selected_entity and sort_by:
        ...
        # Display bar charts and data tables in two columns
