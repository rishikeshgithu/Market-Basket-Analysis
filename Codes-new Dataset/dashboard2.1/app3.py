...
with col3:
    ...
    sort_by = st.selectbox("Sort by:", ['Percentage', 'Confidence', 'Lift', 'Conviction'])
...
if filter_a and selected_entity and sort_by:
    ...
    for entity, df in dataframes.items():
        with table_cols[table_count % 2]:
            st.subheader(f"Items bought with '{selected_entity}' ({entity}) (Frequency, Percentage, Confidence, Lift, and Conviction):")
            st.write(df)
        table_count += 1
