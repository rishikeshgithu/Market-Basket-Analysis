...
@st.cache
def load_data():
    data = pd.read_excel("DS_Round_1_Assignment.xlsx")  # Make sure the file exists
    return data
...
if filter_a:
    ...
    if selected_entity:
        # Calculate support for the selected item (antecedent)
        support_antecedent = data[data[filter_a] == selected_entity].shape[0] / data.shape[0] * 100
        ...
        # Print support of the antecedent
        st.write(f"Support of the selected {filter_a} '{selected_entity}': {support_antecedent:.2f}%")
